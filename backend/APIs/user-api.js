...
userApp.post(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    let dbuser = await usersBalanceObj.findOne({
      username: userCred.usernameTo,
      accountNo: userCred.accountNo,
    });
...
userApp.get(
  ...
  expressAsyncHandler(async (req, res) => {
    const usersCollectionObj = req.app.get("usersCollection");
    let list = await usersCollectionObj
      .find({ score: { $gte: 0 } })
      .sort({ score: -1 })
      .toArray();

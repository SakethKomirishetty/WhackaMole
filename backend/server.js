...
const connectDB = async () => {
  try {
    const conn = await mongoose.connect(
      "mongodb+srv://sakethkomirishetty:n43QzT36RmN1Rtpi@cluster0.lhzrp.mongodb.net/",
      {
...
io.on("connection", (socket) => {
  ...
  socket.on("disconnect", () => {
    ...
    for (let gameId in games) {
      if (
        games[gameId].players.player1.id === socket.id ||
        games[gameId].players.player2.id === socket.id
      ) {
